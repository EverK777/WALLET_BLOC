...
class _CardCreate extends State<CardCreate>{
  ...
  Widget cardColors(CardBloc bloc) {
    final dotSize =
        (MediaQuery.of(context).size.width - 220) / CardColors.baseColors.length;

    List<Widget> dotList = new List<Widget>();

    for (var i = 0; i < CardColors.baseColors.length; i++) {
      dotList.add(
        StreamBuilder<List<CardColor>>(
